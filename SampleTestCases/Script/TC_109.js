...
function EnteringDetails(){
 ...
 function VerifyLogout(){
  Log.AppendFolder("Click on Logout and Verify");
  
  Prop = new Array("idStr", "ObjectIdentifier","ObjectLabel", "ObjectType");
  Val = new Array("logout", "logout","Logout", "Button");
  var LogOutButton = BookingConfirmPage.FindChild(Prop, Val, 100);
  Obj_Click(LogOutButton);
  
  Prop = new Array("namePropStr", "ObjectIdentifier","ObjectLabel", "ObjectType");
  Val = new Array("index.php", "0","Click here to login again", "Link");
  var LoginAgain = LogOutPage.FindChild(Prop, Val, 100);
  
  
  if(LoginAgain.Exists){
   Log.Message("It has been successfully Logged Out.")
  }else{
